...
export function getAllLatestPricesBySource(source: string): PriceRow[] {
  ...
  // ROW_NUMBER with an id tiebreaker (not a MAX(timestamp) join): timestamps
  // have second resolution, so two concurrent inserts for one name would both
  // match a MAX join and double-count in SUM/COUNT consumers.
  return sqlite
    .prepare(
      `SELECT id, market_hash_name, source, price_eur, timestamp FROM (
         SELECT p.*, ROW_NUMBER() OVER (PARTITION BY market_hash_name ORDER BY timestamp DESC, id DESC) AS rn
         FROM prices p
         WHERE source = ?
       ) WHERE rn = 1`,
    )
...
/**
 * Latest price per name for ONE source, restricted to the given names. Batched
 * IN (...) lookup so callers (e.g. dashboard sticker pricing) do a single query
 * instead of one per name. Chunked to stay well under SQLite's bound-parameter
 * limit.
 */
export function getLatestPricesForNames(names: string[], source: string): PriceRow[] {
    ...
    const rows = sqlite
      .prepare(
        `SELECT id, market_hash_name, source, price_eur, timestamp FROM (
           SELECT p.*, ROW_NUMBER() OVER (PARTITION BY market_hash_name ORDER BY timestamp DESC, id DESC) AS rn
           FROM prices p
           WHERE source = ? AND market_hash_name IN (${placeholders})
         ) WHERE rn = 1`,
      )
      .all(source, ...chunk) as PriceRow[];
    out.push(...rows);
  }
  return out;
}
