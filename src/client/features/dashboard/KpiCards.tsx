    ...
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      <Card className="glass-card">
        <CardContent className="p-5">
          <p className="text-xs text-gray-500 uppercase tracking-wider mb-2">{t('dashboard.totalItems')}</p>
          <p className="text-3xl font-extrabold text-white">{totalItems}</p>
          <p className="text-xs text-gray-500 mt-1">{uniqueItems} {t('kpi.uniques')}</p>
        </CardContent>
      </Card>

      <Card className="glass-card">
        <CardContent className="p-5">
          <p className="text-xs text-gray-500 uppercase tracking-wider mb-2">{t('nav.storageUnits')}</p>
          <p className="text-3xl font-extrabold text-cs-purple">{storageUnits.length}</p>
          <p className="text-xs text-gray-500 mt-1">
            {totalItems - mainInventory.count} {t('kpi.storedItems')}
          </p>
        </CardContent>
      </Card>
      ...
      <Card className="glass-card">
        <CardContent className="p-5">
          <p className="text-xs text-gray-500 uppercase tracking-wider mb-2">{t('kpi.totalValue')}</p>
          <p className="text-3xl font-extrabold text-cs-orange">{formatEur(totalValue)}</p>
          <p className="text-xs text-gray-500 mt-1">{t('kpi.steamMarket')}</p>
        </CardContent>
      </Card>

      <Card className="glass-card">
        <CardContent className="p-5">
          <p className="text-xs text-gray-500 uppercase tracking-wider mb-2">{t('kpi.evolution24h')}</p>
          {change24h.hasData ? (
            <>
            ...
            <>
              ...
              <p className="text-xs text-gray-600 mt-1">{t('kpi.noData')}</p>
            </>
          )}
        </CardContent>
      </Card>
